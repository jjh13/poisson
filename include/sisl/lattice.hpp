...
public:
	...
	virtual void SV(const int &x, const int &y, const O &value) = 0;
	...
	virtual void spatialFilter(std::function<O(const int &, const int &, std::function<O(const int &, const int &)>)>) = 0; //
	virtual void frequencyFilter(std::function<O(const I &u, const I &v, const I &h)> lambda) = 0; //
