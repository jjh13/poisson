...
#ifndef _TP_BSPLINELIN2_H_
#define _TP_BSPLINELIN2_H_
...
public:
	...
	inline static const O convolutionSum(const vector2<I> &p, const shift_invariant_space2< tp2linear, O, I> *lattice) {
		O sum = 0;
		...
		for(int i = -1; i <= 2; i++)
			for(int j = -1; j <= 2; j++) {
				sum += lattice->GV(vx + i, vy + j) *
						tp2linear::M(vox.i - I(vx + i), vox.j - I(vy + j));
			}
		return sum;		
	}
